...
class UiTarsAgent(Agent):
    ...
    def __init__(self, model: str = "ui-tars-7b", **kwargs) -> None:
        super().__init__(
            name=model, 
        ...
        expect_env_var("OPENROUTER_API_KEY")
        expect_env_var("OPENROUTER_BASE_URL")

        self.inference_model = "bytedance/ui-tars-1.5-7b"
        self.model = model
        ...
        self.inference_func = self.inference_with_thinking if self.use_thinking else self.inference_without_thinking
    ...
    def inference_with_thinking(self, messages):
        ...
        token_usage = (usage.get("prompt_tokens", 0), usage.get("completion_tokens", 0), usage.get("prompt_token_details", {}).get("cached_tokens", 0))
    ...
    def predict(self, task: str, screenshot: str) -> AgentPredictionResponse:
        ...
        if len(self.history_images) > self.max_images_in_history:
            self.history_images = self.history_images[-self.max_images_in_history:]
        ...
        if len(self.history_responses) > 0:
            for history_idx, history_response in enumerate(self.history_responses):
                ...
                if history_idx + self.max_images_in_history > len(self.history_responses):
                    messages.append({
                        "role": "user",
        ...
        result = AgentPredictionResponse(
            response=prediction,
            pyautogui_actions=parsed_pyautogui_code,
            status=status,
            token_usage=TokenUsage(
                prompt_tokens=usage[0],
                completion_tokens=usage[1],
                cached_prompt_tokens=usage[2],
            )
        )
